...
Alert.show = function (detailString, enterCallBack, needCancel, animSpeed) {
    ...
    // 加载 prefab 创建
    // todo 不进行加载， 初始化的时候传入prefab 
    // properties: { target: {default: null, type: cc.Prefab} }
    // var alert = cc.instantiate(this.target)
    cc.loader.loadRes("Alert", cc.Prefab, function (error, prefab) {
        ...
        var bg = cc.find('alertBackground', alert);
        bg.width = 1000;
        
        var realUrl = cc.url.raw('resources/alert_background.png');
        var texture = cc.textureCache.addImage(realUrl);
        bg.getComponent(cc.Sprite).spriteFrame.setTexture(texture);
        // bg.getComponent(cc.Sprite).type = 'Sliced'
